...
namespace AsterionGame {
 ...
 public sealed class PlayerInputReader : MonoBehaviour {
  public static readonly string[] SlotKeys={"1","2","3","4","Q","F","C","V"};
  InputAction[] extra=new InputAction[5];
  public bool AbilityPressed(int slot)=>slot>=3&&slot<8&&extra[slot-3].WasPressedThisFrame();
  InputAction move, pointer, fire, dash, orbital, pause, restart,target,select,look,mouseDelta;
  ...
  void Update(){
   bool canLook=MouseLookAllowed&&Time.timeScale>0;
   MouseLookActive=canLook&&look.IsPressed();
   SelectPressed=false;
  ...
  public bool PausePressed => pause.WasPressedThisFrame(); public bool RestartPressed => restart.WasPressedThisFrame();
  void Awake() {
   for(int i=0;i<5;i++)extra[i]=new InputAction("Ability"+(i+4),InputActionType.Button,"<Keyboard>/"+SlotKeys[i+3].ToLowerInvariant());
   move = new InputAction("Move",InputActionType.Value); move.AddCompositeBinding("2DVector").With("Up","<Keyboard>/w").With("Down","<Keyboard>/s").With("Left","<Keyboard>/a").With("Right","<Keyboard>/d");
   ...
   look=new InputAction("Mouse look",InputActionType.Button,"<Mouse>/rightButton");
  ...
  InputAction[] Actions() => new[]{move,pointer,fire,dash,orbital,pause,restart,target,select,look,mouseDelta,extra[0],extra[1],extra[2],extra[3],extra[4]};
 }
}
